...
Cloudflare 通过 Git 仓库构建时，构建命令使用：
...
不要把构建环境的 `NODE_ENV` 设置为 `production` 后再省略构建依赖。项目将
Next.js 编译阶段必需的 `typescript`、`@types/node`、`@types/react` 和
`@types/react-dom` 放在 `dependencies` 中，确保 Cloudflare 即使只安装生产依赖也能
完成 TypeScript 构建。修改依赖后请在 Cloudflare 中清除构建缓存并重新部署。
