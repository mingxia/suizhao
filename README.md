...
仓库提交 `pnpm-lock.yaml`，使 Cloudflare 明确选择 pnpm 安装依赖。首次安装会根据
`package.json` 补全 lockfile 中的包快照；`.npmrc` 因此关闭 frozen lockfile，同时允许
npm fallback 忽略 Better Auth 可选的 Lynx React peer dependency 冲突。这样可以避免
依赖安装提前失败、随后在 Next.js 阶段误报未安装 TypeScript。
