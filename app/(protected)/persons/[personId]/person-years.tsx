...
import { stageLabel } from "@/lib/timeline-label";
  ...
  return <>
    <section className="years-grid">
      {cards.map((card) => {
        ...
        const description = locale === "en"
          ? `${personName} — ${type === "family" ? card.stage === "first_seen" ? "wedding photo" : `family photo, year ${card.age}` : card.stage === "first_seen" ? "first photo" : `photo at age ${card.age}`}`
        ...
        return card.photoId
          ? <Link key={key} href={`/persons/${personId}/photos/${pathFor(card)}`} className="year-photo-card card" onClick={(event) => { if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return; event.preventDefault(); openViewer(card); }}>
              ...
              <YearLabel stage={card.stage} age={card.age} year={card.year} type={type} locale={locale} />
            </Link>
      ...
      <div className="year-photo-card year-locked-card card"><div><span aria-hidden="true">♙</span><strong>{locale === "en" ? nextYear : `${nextYear}年`}</strong><small>待解锁</small></div></div>
    </section>
    ...
    {viewCard?.photoId && <div className="photo-viewer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && closeViewer()}>
      <section className="photo-viewer" role="dialog" aria-modal="true" aria-labelledby="photo-viewer-title">
        ...
        <div className="photo-viewer-caption">
          <div><p className="modal-eyebrow">{personName} · {type === "family" ? "家庭相册" : "成长相册"}</p><h2 id="photo-viewer-title">{stageLabel(type, viewCard.stage, viewCard.age, locale)} <span>/ {viewCard.year}</span></h2>{viewCard.note && <p className="photo-viewer-note">{viewCard.note}</p>}</div>
    ...
    {uploadCard !== null && <div className="upload-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setUploadCard(null)}>
      <section className="upload-modal card" role="dialog" aria-modal="true" aria-labelledby="upload-title">
        ...
        <p className="modal-eyebrow">{personName} · {stageLabel(type, uploadCard.stage, uploadCard.age, locale)}</p>
...
function YearLabel({ stage, age, year, type, locale }: { stage: YearPhotoStage; age: number | null; year: number; type: "person" | "family"; locale: "zh" | "en" }) {
  return <p className="year-label"><strong>{stageLabel(type, stage, age, locale)}</strong><span>/</span><time>{year}</time></p>;
}
