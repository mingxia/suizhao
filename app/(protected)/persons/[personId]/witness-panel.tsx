...
import { createWitness, deleteWitness, type WitnessActionState } from "@/actions/witness-actions";

type WitnessItem = { id: string; name: string; relation: string; permission: "readonly" | "comment" | "family"; token: string; lastVisitedAt: string | null; viewedYears: number[] };
  ...
  return <section className="witness-panel card">
    ...
    {items.length === 0 ? <div className="witness-empty"><span>♡</span><p>邀请重要的人，一起见证{personName}的成长。</p><small>家人无需注册，打开专属链接就能查看与祝福。</small></div> : <div className="witness-list">
      {items.map((item) => <article className="witness-row" key={item.id}>
        ...
        <div className="witness-main"><strong>{item.name}</strong><span>{item.relation} · {permissionNames[item.permission]}</span>
          <small>{item.lastVisitedAt ? `最近访问：${formatDate(item.lastVisitedAt)}` : "还没有来过"}{item.viewedYears.length ? ` · 浏览：${item.viewedYears.join("、")}` : ""}</small>
        </div>
        <button className="witness-copy" type="button" onClick={() => copy(item.token)}>{copied === item.token ? "已复制 ✓" : "复制专属链接"}</button>
