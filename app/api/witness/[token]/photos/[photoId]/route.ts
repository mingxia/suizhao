...
export async function GET(request: Request, { params }: { params: Promise<{ token: string; photoId: string }> }) {
  ...
  const [row] = await (await getDb()).select({ photo: yearPhotos }).from(yearPhotos)
  ...
  return new Response(object.body, { headers: { "Content-Type": "image/webp", "Cache-Control": "private, max-age=3600", ETag: object.httpEtag, "X-Content-Type-Options": "nosniff", "Referrer-Policy": "no-referrer" } });
}
