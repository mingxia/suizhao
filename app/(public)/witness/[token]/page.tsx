...
export default async function WitnessPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const now = new Date();
  const viewedYears = [...new Set(photos.map((photo) => photo.year))];
  await Promise.all([
    db.insert(witnessVisits).values({ id: crypto.randomUUID(), witnessId: record.witness.id, visitedAt: now, ip: requestHeaders.get("cf-connecting-ip") ?? requestHeaders.get("x-forwarded-for")?.split(",")[0]?.trim(), userAgent: requestHeaders.get("user-agent"), viewedYears: JSON.stringify(viewedYears) }),
      ...
      {photos.map((photo, index) => {
        ...
        return <article className="witness-year" key={photo.id}>
          ...
          <div className="witness-memory-card">
            <img src={`/api/witness/${token}/photos/${photo.id}?variant=large`} alt={`${record.person.name}${photo.stage === "first_seen" ? "初见" : `${photo.age}岁`}的照片`} loading={index ? "lazy" : "eager"} />
