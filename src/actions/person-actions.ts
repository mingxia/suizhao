...
export async function updatePerson(personId:string, formData:FormData):Promise<ActionResult>{ const session=await requireSession(); await requirePersonOwner(personId,session.user.id); const parsed=personSchema.safeParse(Object.fromEntries(formData)); if(!parsed.success) return {success:false,error:{code:"INVALID_INPUT",message:"请检查人物资料"}}; const [highest]=await (await getDb()).select({age:max(yearPhotos.age)}).from(yearPhotos).where(eq(yearPhotos.personId,personId)); if((highest?.age??0)>getCurrentAge(parsed.data.birthday)) return {success:false,error:{code:"INVALID_INPUT",message:"修改后的出生日期与现有年龄照片不一致，请先处理相关照片。"}}; await (await getDb()).update(persons).set({...parsed.data,nickname:parsed.data.nickname||null,updatedAt:new Date()}).where(eq(persons.id,personId)); revalidatePath(`/persons/${personId}`); return {success:true,data:undefined}; }
